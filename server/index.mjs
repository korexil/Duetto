    ...
    const _pr = (async () => {
      try {
        ...
        try {
          ...
          if (!aurl) { try { const su = await ncm.song_url({ id: sid, cookie: ncmCookie, unblock: 'true' }); aurl = (su.body && su.body.data && su.body.data[0] && su.body.data[0].url) || ''; } catch(e){} }
...
app.get('/api/ncm/song-url', async (q,r)=>{ try{ const su=await ncm.song_url_v1({ id:q.query.id, level:'standard', cookie:ncmCookie, unblock:'true' }); const u=su.body&&su.body.data&&su.body.data[0]; let url=u&&u.url||''; if(url) url=url.replace(/^http:/,'https:'); r.json({ ok:true, url }); }catch(e){ r.status(500).json({ok:false,error:String(e.message||e)}); } });
